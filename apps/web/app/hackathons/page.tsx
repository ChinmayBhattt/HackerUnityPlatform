...
export default function HackathonsDirectoryPage() {
  ...
  const filteredEvents = events
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'popular') return b.participantsCount - a.participantsCount;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full flex-1">
      ...
      <div className="mb-8">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-50 border border-sky-200 text-[#0099e6] text-xs font-bold uppercase tracking-wider mb-2">
          ...
          <span>Global Hackathons Directory</span>
        </div>
        <h1 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight">
          Explore Hackathons & Sprints
        </h1>
        <p className="text-xs sm:text-sm text-slate-500 mt-2 max-w-2xl font-medium">
          Browse through {events.length} curated hackathons with escrow-backed prize pools, world-class judges, and vibrant developer communities.
        </p>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm mb-8 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search hackathons by keyword, tech stack (#Agents, #ZK), or host..."
              value={searchQuery}
      ...
      <div className="flex items-center justify-between mb-6 text-xs text-slate-500 font-medium">
        <span>
          Showing <span className="font-bold text-slate-900">{filteredEvents.length}</span> hackathons
        </span>
        <span className="text-[11px] text-slate-400">All prize funds guaranteed in escrow</span>
      </div>
