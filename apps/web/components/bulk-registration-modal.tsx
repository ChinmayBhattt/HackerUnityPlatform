...
export function BulkRegistrationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#0c1017] rounded-3xl border border-slate-200 dark:border-white/[0.08] shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="p-6 border-b border-slate-100 dark:border-white/[0.08] flex items-start justify-between gap-4 bg-slate-50/50 dark:bg-white/[0.02]">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-blue-50 dark:bg-sky-950/40 text-[#0099e6] border border-blue-100 dark:border-sky-850/40 flex items-center gap-1">
                <Sparkles className="w-3 h-3" />
                Bulk Importer
              </span>
              <span className="text-xs text-slate-400 dark:text-slate-500 font-mono">CSV / TSV / Excel</span>
            </div>
            <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">
              Bulk Upload Registrations
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Import participant data in bulk for <strong className="text-slate-800 dark:text-slate-200">{eventName}</strong>.
            </p>
          </div>

          <button
            onClick={onClose}
            className="p-2 rounded-xl text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200/80 dark:border-white/[0.08]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200">Need the correct column format?</h4>
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-white dark:bg-[#121824] border border-slate-200 dark:border-white/[0.08] hover:bg-slate-100 dark:hover:bg-white/[0.06] text-slate-700 dark:text-slate-200 text-xs font-bold transition-all shadow-2xs flex items-center gap-1.5 shrink-0 cursor-pointer"
            >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            {/* Tabs */}
            <div className="flex p-1 bg-slate-100 dark:bg-[#121824] rounded-xl max-w-fit border border-transparent dark:border-white/[0.08]">
              <button
                type="button"
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Assign Status:</span>
              <select
                value={defaultStatus}
                onChange={(e) => {
                ...
                className="px-3 py-1.5 bg-slate-50 dark:bg-[#121824] border border-slate-200 dark:border-white/[0.08] rounded-xl text-xs font-bold text-slate-800 dark:text-slate-200 outline-none focus:border-[#0099e6] dark:focus:border-[#0099e6] cursor-pointer"
              >
            ...
            <div className="space-y-3">
              ...
              {!file ? (
                <div
                  ...
                  className="border-2 border-dashed border-slate-200 dark:border-white/[0.1] hover:border-[#0099e6] dark:hover:border-[#0099e6] bg-slate-50/50 dark:bg-white/[0.01] hover:bg-blue-50/30 dark:hover:bg-sky-950/20 rounded-2xl p-8 text-center cursor-pointer transition-all flex flex-col items-center justify-center space-y-2.5 group"
                >
                  <div className="w-12 h-12 rounded-2xl bg-white dark:bg-[#121824] shadow-xs border border-slate-200 dark:border-white/[0.08] flex items-center justify-center text-slate-400 dark:text-slate-500 group-hover:text-[#0099e6] group-hover:scale-105 transition-all">
                ...
                <div className="flex items-center justify-between p-3.5 rounded-2xl bg-blue-50/50 dark:bg-sky-950/30 border border-blue-100 dark:border-sky-850/40">
                  <div className="flex items-center gap-3 truncate">
                    <div className="w-9 h-9 rounded-xl bg-blue-100 dark:bg-sky-900/40 text-[#0099e6] flex items-center justify-center shrink-0">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-white dark:hover:bg-white/10 transition-colors cursor-pointer"
                  >
          ...
          {activeTab === 'paste' && (
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full p-3.5 bg-slate-50 dark:bg-[#121824] border border-slate-200 dark:border-white/[0.08] rounded-2xl font-mono text-xs text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:border-[#0099e6] dark:focus:border-[#0099e6] focus:bg-white dark:focus:bg-[#121824] transition-all resize-none"
              />
          ...
          {/* Parse error */}
          {parseError && (
            <div className="p-3.5 rounded-2xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/40 text-red-700 dark:text-red-300 text-xs flex items-center gap-2">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/40">
                    {validCount} Valid
                  </span>
                  {invalidCount > 0 && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-red-50 dark:bg-red-950/40 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800/40">
              ...
              <div className="border border-slate-200 dark:border-white/[0.08] rounded-2xl overflow-hidden shadow-2xs">
                <div className="max-h-48 overflow-y-auto">
                  <table className="w-full text-left text-[11px]">
                    <thead className="bg-slate-50 dark:bg-white/[0.03] border-b border-slate-200 dark:border-white/[0.08] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider sticky top-0">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-white/[0.06] text-slate-700 dark:text-slate-300">
                      {parsedRows.slice(0, 5).map((row, idx) => (
                        <tr
                          key={idx}
                          className={row.valid ? 'hover:bg-slate-50/60 dark:hover:bg-white/[0.02]' : 'bg-red-50/40 dark:bg-red-950/30 text-red-800 dark:text-red-300'}
                        >
                          <td className="py-2 px-3">
                            {row.valid ? (
                              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                            ) : (
                              <span title={row.error} className="flex items-center text-red-600 dark:text-red-400">
                          ...
                          <td className="py-2 px-3 text-slate-500 dark:text-slate-400">{row.phone || '—'}</td>
                          <td className="py-2 px-3 text-slate-500 dark:text-slate-400">{row.college || '—'}</td>
                          <td className="py-2 px-3">
                            {row.skills && row.skills.length > 0 ? (
                              <span className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-white/[0.06] text-slate-600 dark:text-slate-300 text-[10px]">
                                {row.skills.join(', ')}
                              </span>
                            ) : (
                              '—'
        ...
        {/* Footer */}
        <div className="p-4 border-t border-slate-100 dark:border-white/[0.08] bg-slate-50/50 dark:bg-white/[0.02] flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={onClose}
